...
class SAC_Trainer(Policy):
    ...
    def __init__(self, file_name):
        ...
        self.actor, self.critic1, self.critic2, self.temperature = self.agent.actor,self.agent.critic, self.agent.critic2, self.agent.Temperature
    ...
    def generate_optimizer(self):
        ...
        weights = list(self.actor.parameters())
        ...
        temperature_optimizer = get_optimizer(self.optimizer_parms['actor'], self.temperature)
    ...
    def get_action_deterministic(self,state):
        ...
        state = state.view(size)
        output = torch.tanh(self.actor(state))
        mean = output[:,:-1]
    ...
    def train(self):
        ...
        for i in range(self.batch_size):
            ...
            next_actions.append(mini_batch[i][5])
        actions = torch.tensor(actions).view(self.batch_size,self.action_size).to(self.gpu).float()
        # next_actions = torch.tensor(next_actions).view(self.batch_size, self.action_size).to(self.gpu).float()
        ...
        temperature = self.temperature(torch.ones(1).to(self.gpu).view(-1,1).float())
