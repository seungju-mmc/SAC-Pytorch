...
class SAC_Agent(Base_Agent):
    ...
    def __init__(self, parms):
        ...
        self.t_act = self.temp_parm['activation']
        self.t_n_unit = self.temp_parm['filter_size']
        self.t_bn = self.temp_parm['batch_norm'] == 'True'
    ...
    def forward(self, state):
        ...
        cov = torch.eye(self.action_size).to(self.device).matmul(std)
        normal = torch.distributions.multivariate_normal.MultivariateNormal(mean, cov)
        x_t = normal.rsample()
    ...
    def loss(self, states, targets,actions,alpha = 0):
        ...
        temperature = self.Temperature(torch.ones(1).to(self.device).view(-1,1).float())*self.t_scaling+self.t_offset

        temperature_ = temperature.detach()
        ...
        loss_temp = torch.mean(temperature * (-log_prob_+ self.action_size))
